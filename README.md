...
8. Open **Sessions** to replay, export through the native share sheet, delete recordings, or transcribe them on supported devices.

The native app requires iOS 18 or newer and uses only SwiftUI, WebKit, Network, AVFoundation, Speech, and system file APIs. It retains the timestamp and session model used by the browser application. Native recordings and the last selected source are excluded from device backup and never leave the device. On iOS 26 or newer, **Transcribe** uses Apple's `SpeechAnalyzer` and English `SpeechTranscriber` model entirely on-device; the system downloads and manages that language asset when necessary. Older iOS versions retain recording and playback but do not show native transcription.
